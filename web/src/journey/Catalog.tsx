...
export default function Catalog(
  { entries, ordered, loading, flying, bursting, gestations, hatching, fresh, returning, confirming, onPick, onCancel, onResume, onReanalyze }:
  {
    ...
    onReanalyze?: (slug: string, title: string) => void;
  },
) {
  useEffect(() => { try { localStorage.removeItem("hyenovel:catalog-pos"); } catch { /* ignore */ } }, []);
  // 哪顆已完成星正在「重新分析」二次確認中(一次一顆;會 snapshot 重跑、燒訂閱,故要確認)
  const [confirmRe, setConfirmRe] = useState<string | null>(null);
      ...
      {ordered.map((slug, i) => {
        ...
        return (
          <div className={cls} data-testid="story" key={slug} style={style}
            ...
            onMouseLeave={() => setConfirmRe(c => (c === slug ? null : c))}>
            ...
            {gest
              ? (dormant
                  ? (hasBone
                      ? <StoryBone slug={slug} hasViz dormant={dormant} />
                      : <CloudCollapse width={300} frozen tone={dormant} />)
                  : (gest.vizReady
            ...
            {gest && !dormant && gest.step < 4 && (
              <button className="gest-x" onClick={e => { e.stopPropagation(); onCancel(slug); }}>✕ 取消</button>
            )}
            {/* 時間不寫在星上:用量上限是全域狀態,右下角 usage-toast 已掛「HH:MM 重置」,
                多顆 paused 星共用同一時刻 → 星上再寫就是重複。星只講自己的狀態(色 + 續跑)。 */}
            {gest && dormant && (
              <div className="gest-word dorm">
                {dormant === "paused"
                  ? "暫停"
                  : `已中斷${gest.reason && FAIL_WORD[gest.reason] ? ` · ${FAIL_WORD[gest.reason]}` : ""}`}
              </div>
            )}
            {gest && dormant && (
              <button className="gest-resume" onClick={e => { e.stopPropagation(); onResume?.(slug, gest.title); }}>續跑</button>
            )}
            {/* 已完成星:hover 才浮出「重新分析」(不佔常駐版面);點了原地二次確認——
                reanalyze 會 snapshot 舊版到 .prev 再重跑一遍(燒訂閱),故不讓手滑直接觸發。 */}
            {!gest && entry?.has_feedback && entry?.has_viz && onReanalyze && (
              confirmRe === slug
                ? <div className="reanalyze-confirm">
                    <span>重跑一遍?</span>
                    <button onClick={e => { e.stopPropagation(); onReanalyze(slug, entry.title); setConfirmRe(null); }}>確定</button>
                    <button onClick={e => { e.stopPropagation(); setConfirmRe(null); }}>取消</button>
                  </div>
                : <button className="reanalyze" onClick={e => { e.stopPropagation(); setConfirmRe(slug); }}>↻ 重新分析</button>
            )}
          </div>
        );
      })}
