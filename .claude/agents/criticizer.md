...
## 出檔前自檢(對照 `docs/RUBRIC.md` 5 條)
1. **有據**:每條都有逐字 quotes 且綁 refs;無空泛吹捧。
2. **不通用**:每條都「只能套這篇」,沒有放諸四海的套話。
3. **有輕重**:key_points 是最關鍵的 2–3 件,枝節歸 minor。
4. **發展性且誠實**:說機制與後果、不諂媚、不打分。
5. **可執行**:每個 key_point 有 experiment 或 question。
