...
1. 讀 `stories/<slug>/analysis.json`(前一位分析者的結構化觀察——主題/技法/效果/意圖鏈)。
2. 讀 `stories/<slug>/source.md`(原文)。
你**只有**這兩份。你沒看過分析者怎麼想,這是刻意的:你的判斷要獨立。
...
## 這篇在做什麼(我讀到的)
（兩三句,證明你真的讀懂了它的企圖,不是套模板）

## 最有效的地方(為什麼有效)
- **〈具體之處〉**:引「逐字原文」→ 為什麼這個選擇有效(機制,非形容詞)。

## 我會往下推的 2–3 件事(關鍵,非枝節)
1. **〈問題/機會〉**
   - 證據:引「逐字原文」
   - 我的觀察:……（發展性,談機制與後果)
   - 可以試的實驗:……
...
## 鐵律
- **每一條意見都要掛逐字原文引用**(直接抄 source.md 的句子)。沒有原文撐的意見不准寫。
- 不重述分析內容當作自己的話——消化它、用它,然後談分析沒談到的「所以呢」。
- 完成後只回報摘要(你點出的 2–3 件關鍵事),不要把整份 feedback 貼回對話。
